...
export const AdminDashboard: React.FC = () => {
  ...
  const ticketStudentName = (ticket: SupportTicket) => {
    ...
    const application = applications.find(item => item.id === ticket.assignedStudentId);
    return application ? `${application.firstName} ${application.lastName}` : 'Unknown student';
  };
  const formatTicketTimestamp = (value?: string) => {
    if (!value) return 'Previously assigned';
      ...
      {activeTab === 'TICKETS' && (
        <div className="space-y-6 animate-in fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {ticketTemplates.length === 0 && <div className="md:col-span-2 p-10 text-center border border-dashed border-[#D0D0D0] rounded-2xl text-sm text-[#707070]">Create a ticket scenario to assign it automatically to each student.</div>}
          </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.35fr)_minmax(360px,0.65fr)] min-h-[680px]">
              <section className="bg-[#202020] p-4 overflow-auto flex flex-col">
                ...
                <div className="flex-1 min-h-[580px] bg-[#333] rounded-xl overflow-auto flex items-center justify-center">
                  {popPreviewLoading ? <p className="text-white text-xs">Loading secure preview…</p> : popPreview?.type === 'application/pdf' ? <iframe title="POP PDF preview" src={popPreview.url} className="w-full h-[650px] bg-white" /> : popPreview?.type.startsWith('image/') ? <img src={popPreview.url} alt="Submitted proof of payment" onError={event => { event.currentTarget.style.display = 'none'; showToast('error', 'POP Preview Unavailable', 'The stored proof could not be decoded as an image. Download the document or check the API storage logs.'); }} className="max-w-none transition-transform duration-200" style={{ transform: `rotate(${popRotation}deg) scale(${popZoom})`, maxHeight: popZoom <= 1 ? '620px' : 'none' }} /> : <p className="text-white text-xs">Preview unavailable for this file format.</p>}
                </div>
              </section>
