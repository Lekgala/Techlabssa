...
app.post('/api/auth/request-access', rateLimit('request-access', 5, 15 * 60 * 1000), async (req, res) => {
  ...
  const delivery = await sendEmail({ to: email, subject, html: `<h2>${escapeHtml(subject)}</h2><p>Hello ${escapeHtml(application.firstName)},</p><p>Use the secure link below to ${escapeHtml(action)}.</p><p><a href="${escapeHtml(link)}" style="display:inline-block;padding:12px 18px;background:#000;color:#fff;text-decoration:none;border-radius:8px">Continue securely</a></p><p>This link can be used once and expires in ${purpose === 'MAGIC_LOGIN' ? '15 minutes' : '1 hour'}. Open it in the same browser you use for the student portal.</p><p>If you did not request this link, you can ignore this email. If it expires, request a new one from the student sign-in page.</p><p>Regards,<br>TechLabs Academy</p>` });
...
app.post('/api/admin/payments/:id/reject', authenticate, requireRole('ADMIN'), async (req: AuthedRequest, res) => {
  ...
  if (application && invoice) {
    ...
    emailDelivery = await sendEmail({ to: application.email, subject, html: `<h2>Proof of payment needs attention</h2><p>Hello ${escapeHtml(application.firstName)},</p><p>We could not verify the proof of payment you uploaded.</p><p><strong>Reason:</strong> ${escapeHtml(reason)}</p><h3>What to do next</h3><ol><li>Sign in to the student portal and check the invoice number, required amount, and banking details.</li><li>Upload one clear, bank-generated proof of payment as a PDF, JPG, or PNG using the correct invoice reference.</li><li>Check the portal for an update before uploading another copy.</li></ol><p>Your balance has not changed. If you need help understanding the reason above, reply to this email with your invoice number.</p><p>Regards,<br>TechLabs Academy</p>` });
...
const approveAndEmail = async (req: AuthedRequest, applicationId: string) => {
  ...
  const paymentInstructions = invoice.paymentMethod === 'EFT'
    ? `<h3>Steps 2 and 3: Pay and upload your proof</h3><p><strong>Bank:</strong> ${escapeHtml(db.academySettings.bankName)}<br><strong>Account name:</strong> ${escapeHtml(db.academySettings.accountName)}<br><strong>Account number:</strong> ${escapeHtml(db.academySettings.accountNumber)}<br><strong>Branch code:</strong> ${escapeHtml(db.academySettings.branchCode)}<br><strong>EFT reference:</strong> ${escapeHtml(invoice.invoiceNumber)}</p><p>Pay the amount shown on the attached invoice by ${escapeHtml(invoice.dueDate)}. Then sign in to the portal and upload the bank-generated proof of payment as a PDF, JPG, or PNG.</p>`
    : `<h3>Steps 2 and 3: Pay securely with Yoco</h3><p>Sign in to the student portal and use the Yoco card payment option to pay the amount shown on the attached invoice by ${escapeHtml(invoice.dueDate)}. Your invoice and enrollment status update automatically after Yoco confirms payment.</p>`;
  const pdf = await generateInvoicePDF({ invoiceNumber: invoice.invoiceNumber, invoiceDate: invoice.invoiceDate || application.submissionDate, dueDate: invoice.dueDate, studentName: invoice.studentName, studentEmail: invoice.studentEmail, studentPhone: application.whatsapp, studentCity: application.city, amount: invoice.amountZAR, description: 'TechLabs Academy IT Support Bootcamp tuition', reference: invoice.invoiceNumber, companyName: db.academySettings.companyName || db.academySettings.academyName, academyName: db.academySettings.academyName, companyAddress: db.academySettings.campusAddress || db.academySettings.location, admissionsEmail: db.academySettings.admissionsEmail, courseTier: invoice.courseTier, paymentMethod: invoice.paymentMethod, listPrice: invoice.listPriceZAR, discountAmount: invoice.discountZAR, discountPercent: invoice.discountPercent, paidAmount: invoice.paidZAR ?? 0, balanceAmount: invoice.balanceZAR, bankName: db.academySettings.bankName, accountName: db.academySettings.accountName, accountNumber: db.academySettings.accountNumber, branchCode: db.academySettings.branchCode, paymentTerms: [invoice.paymentOption === 'FULL' ? 'Full tuition is payable by the due date.' : `A seat deposit of R${invoice.depositZAR.toLocaleString('en-ZA')} is required.`, invoice.paymentMethod === 'EFT' ? 'Use the invoice number as the EFT reference.' : 'Pay securely by card through the student portal.', 'Payments are confirmed automatically after provider confirmation.'], installments: db.paymentInstallments.filter(item => item.invoiceId === invoice.id).sort((a, b) => a.sequence - b.sequence), payments: [] });
...
app.post('/api/invoices/:id/email', authenticate, requireRole('ADMIN'), async (req, res) => {
  ...
  const pdf = await generateInvoicePDF({
    ...
    paymentTerms: [
      ...
      ...(invoice.paymentMethod === 'EFT' ? ['Use the invoice number as the EFT payment reference.', 'Upload the bank-generated proof of payment in the student portal. Payments remain unconfirmed until verified by admissions.'] : ['Pay securely by card through the student portal. Yoco confirms successful payments automatically.']),
  ...
  const delivery = await sendEmail({
    ...
    subject,
    html: `<div style="font-family:Arial,sans-serif;max-width:640px;margin:auto;color:#1a1a1a"><h2>Your TechLabs Academy invoice</h2><p>Hello ${escapeHtml(application?.firstName || invoice.studentName)},</p><p>Your PDF invoice is attached. The summary and EFT details are below for convenience.</p><table style="width:100%;border-collapse:collapse"><tr><td style="padding:8px;border-bottom:1px solid #ddd">Invoice</td><td style="padding:8px;border-bottom:1px solid #ddd;text-align:right"><strong>${escapeHtml(invoice.invoiceNumber)}</strong></td></tr><tr><td style="padding:8px;border-bottom:1px solid #ddd">Course tier</td><td style="padding:8px;border-bottom:1px solid #ddd;text-align:right">${escapeHtml(invoice.courseTier)}</td></tr><tr><td style="padding:8px;border-bottom:1px solid #ddd">Total tuition</td><td style="padding:8px;border-bottom:1px solid #ddd;text-align:right">R${invoice.amountZAR.toLocaleString('en-ZA')}</td></tr><tr><td style="padding:8px;border-bottom:1px solid #ddd">Paid to date</td><td style="padding:8px;border-bottom:1px solid #ddd;text-align:right">R${paidZAR.toLocaleString('en-ZA')}</td></tr><tr><td style="padding:8px;border-bottom:1px solid #ddd">Remaining balance</td><td style="padding:8px;border-bottom:1px solid #ddd;text-align:right"><strong>R${invoice.balanceZAR.toLocaleString('en-ZA')}</strong></td></tr><tr><td style="padding:8px;border-bottom:1px solid #ddd">Amount currently required</td><td style="padding:8px;border-bottom:1px solid #ddd;text-align:right"><strong>R${nextAmount.toLocaleString('en-ZA')}</strong></td></tr><tr><td style="padding:8px">Due date</td><td style="padding:8px;text-align:right">${escapeHtml(invoice.dueDate)}</td></tr></table><h3>EFT banking details</h3><p><strong>Bank:</strong> ${escapeHtml(db.academySettings.bankName)}<br><strong>Account name:</strong> ${escapeHtml(db.academySettings.accountName)}<br><strong>Account number:</strong> ${escapeHtml(db.academySettings.accountNumber)}<br><strong>Branch code:</strong> ${escapeHtml(db.academySettings.branchCode)}<br><strong>Payment reference:</strong> ${escapeHtml(invoice.invoiceNumber)}</p><p>After payment, sign in to your student portal and upload the bank-generated proof of payment.</p><p>Regards,<br>TechLabs Academy</p></div>`,
    attachments: [{ filename: `${invoice.invoiceNumber.replace(/[^A-Za-z0-9_-]/g, '-')}.pdf`, content: pdf }],
  });
...
app.post('/api/bulk/send-emails', authenticate, requireRole('ADMIN'), async (req, res) => {
  ...
  const db = await getDatabase(); const template = db.emailTemplates.find(item => item.id === req.body?.templateId)
    || (req.body?.templateId === 'tpl-lead-marketing' ? { id: 'tpl-lead-marketing', name: 'Lead Marketing Campaign', trigger: 'MANUAL_SEND', subject: 'TechLabs Academy: explore your next IT career step', htmlBody: '<table role="presentation" cellpadding="0" cellspacing="0" style="width:100%;border-collapse:collapse;background:#f4f5f7;font-family:Arial,Helvetica,sans-serif;color:#1a1a1a"><tr><td align="center" style="padding:24px 12px"><table role="presentation" cellpadding="0" cellspacing="0" style="width:100%;max-width:600px;border-collapse:collapse;background:#ffffff;border:1px solid #e1e1e1"><tr><td style="padding:22px 28px;background:#000000;color:#ffffff;font-size:18px;font-weight:700;letter-spacing:1px">◆ TECHLABS <span style="color:#bdbdbd;font-size:10px;letter-spacing:2px">SA</span></td></tr><tr><td style="padding:36px 28px 28px"><p style="margin:0 0 12px;color:#707070;font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase">A practical next step in IT</p><h1 style="margin:0 0 18px;color:#111111;font-size:30px;line-height:1.15;font-weight:700">Build skills that work in the real world.</h1><p style="margin:0 0 18px;color:#444444;font-size:16px;line-height:1.7">Hello {studentName}, explore upcoming TechLabs Academy cohorts, hands-on labs, and practical IT support training built for the workplace.</p><p style="margin:0 0 24px;color:#444444;font-size:15px;line-height:1.6">See what is available for the next intake and choose the path that fits your goals.</p><p style="margin:0 0 8px"><a href="https://techlabssa.vercel.app/pricing" style="display:inline-block;padding:13px 20px;background:#000000;color:#ffffff;text-decoration:none;font-size:13px;font-weight:700;letter-spacing:.5px">Explore programmes</a></p><p style="margin:20px 0 0;color:#707070;font-size:13px;line-height:1.6">Questions? Reply to this email or contact our admissions team.</p></td></tr><tr><td style="padding:18px 28px;border-top:1px solid #e5e5e5;color:#8a8a8a;font-size:11px;line-height:1.5">TechLabs Academy SA · Practical IT training in South Africa</td></tr></table></td></tr></table>', enabled: true, variables: ['studentName'] } : null);
