...
const approveAndEmail = async (req: AuthedRequest, applicationId: string) => {
  ...
  const accountInstructions = `<h3>Create your portal password</h3><p><a href="${escapeHtml(setupLink)}" style="display:inline-block;padding:13px 20px;background:#111111;color:#ffffff;text-decoration:none;border-radius:8px;font-weight:700">Create portal password</a></p><p>This private link can be used once and expires in 24 hours.</p>`;
  const paymentInstructions = `<h3>Choose how to pay</h3><p>After setting your password, open Payments in your student portal. You can pay by card through Yoco or use the EFT details shown there. Card payments are confirmed automatically; an EFT payment requires a bank-generated proof of payment.</p><p>Use <strong>${escapeHtml(invoice.invoiceNumber)}</strong> as your EFT reference. The amount and due date are on your attached invoice.</p>`;
...
app.post('/api/invoices/:id/email', authenticate, requireRole('ADMIN'), async (req, res) => {
  ...
  const delivery = await sendEmail({
    ...
    subject,
    html: `<h2>Your invoice is ready</h2>
      <p>Hello ${escapeHtml(application?.firstName || invoice.studentName)},</p>
      <p>Your updated invoice is attached as a PDF.</p>
      <table role="presentation" style="width:100%;border-collapse:collapse;margin:20px 0;background:#f5f7f8">
        <tr><td style="padding:10px 14px">Invoice</td><td style="padding:10px 14px;text-align:right"><strong>${escapeHtml(invoice.invoiceNumber)}</strong></td></tr>
        <tr><td style="padding:10px 14px">Total tuition</td><td style="padding:10px 14px;text-align:right">R${invoice.amountZAR.toLocaleString('en-ZA')}</td></tr>
        <tr><td style="padding:10px 14px">Paid to date</td><td style="padding:10px 14px;text-align:right">R${paidZAR.toLocaleString('en-ZA')}</td></tr>
        <tr><td style="padding:10px 14px">Remaining balance</td><td style="padding:10px 14px;text-align:right"><strong>R${invoice.balanceZAR.toLocaleString('en-ZA')}</strong></td></tr>
        <tr><td style="padding:10px 14px">Amount currently due</td><td style="padding:10px 14px;text-align:right"><strong>R${nextAmount.toLocaleString('en-ZA')}</strong></td></tr>
        <tr><td style="padding:10px 14px">Due date</td><td style="padding:10px 14px;text-align:right">${escapeHtml(invoice.dueDate)}</td></tr>
      </table>
      <p>To pay, open Payments in your student portal. Choose secure card checkout or use the EFT banking details shown there. Card payments are confirmed automatically; for EFT, upload your bank-generated proof of payment. Use <strong>${escapeHtml(invoice.invoiceNumber)}</strong> as your EFT reference.</p>
      <p>If you have already paid, check your portal before making another payment.</p>
...
app.post('/api/automation/render', authenticate, requireRole('ADMIN'), async (req, res) => {
  ...
  rendered ? res.json({ ...rendered, html: formatEmailHtml(rendered.html) }) : res.status(404).json({ error: 'Template not found' });
});
...
app.post('/api/bulk/send-emails', authenticate, requireRole('ADMIN'), async (req, res) => {
  ...
  const db = await getDatabase(); const template = db.emailTemplates.find(item => item.id === req.body?.templateId)
    || (req.body?.templateId === 'tpl-lead-marketing' ? DEFAULT_EMAIL_TEMPLATES.find(item => item.id === 'tpl-lead-marketing') : null);
