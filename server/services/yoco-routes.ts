...
async function sendYocoConfirmationEmail(intentId: string): Promise<void> {
  ...
  const delivery = await sendEmail({
    to: student.email,
    subject,
    html: `<h2>${intent.mode === 'test' ? 'Test payment confirmed' : 'Card payment confirmed'}</h2><p>Hello ${escapeHtml(student.firstName)},</p><p>Yoco confirmed your ${intent.mode === 'test' ? 'test ' : ''}card payment of <strong>R${(intent.amountCents / 100).toLocaleString('en-ZA', { minimumFractionDigits: 2 })}</strong>.</p><p style="padding:14px 18px;background:#f5f7f8;border-left:3px solid #80cdf0"><strong>Invoice reference:</strong> ${escapeHtml(invoice.invoiceNumber)}<br><strong>Remaining balance:</strong> R${invoice.balanceZAR.toLocaleString('en-ZA', { minimumFractionDigits: 2 })}</p><p>Your invoice and receipt are available in the student portal. No proof of payment is needed for a card payment.</p><p><a href="${escapeHtml(`${process.env.APP_ORIGIN || 'http://localhost:3000'}/student/payments`)}" style="display:inline-block;padding:13px 20px;background:#111111;color:#ffffff;text-decoration:none;border-radius:8px;font-weight:700">View payments</a></p><p><small>Yoco transaction: ${escapeHtml(intent.paymentId || intent.id)}</small></p><p>Regards,<br>TechLabs Academy</p>`,
  });
